...
public class CounterMinTest {
  @Test
  void test() {
    int k = 3, w = 3000;
    CountMin classUnderTest = new CountMin(k, w);
    record(classUnderTest);
    query(classUnderTest);
  }

  private void record(CountMin cmin) {
  ...
  private void query(CountMin cmin) {
    BufferedReader reader;
    int err = 0;
    try {
      reader = new BufferedReader(new FileReader("src/test/resources/project3input.txt"));
      ...
      reader.close();
      File outFile = new File("src/test/resources/counter/countMin.txt");
